...
A toggle never clears a cursor. The first round after re-enabling sends the stored cursor, and starts a full sync when the provider rejects it. Both providers already force that branch: an expired cursor comes back as `410` and the client resynchronizes without one (#1175 section 4, #1176 section 7), so re-enabling needs no code path scheduled sync lacks.
