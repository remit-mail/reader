...
Biome keeps a `packages/workbench/src` prototype built from shipped parts. `noRestrictedImports` lets it import `@remit/ui` and the web-client story frame (`@/mocks/story-frame`), and nothing else of ours: no other `@remit` package, no other `@/` path, no relative path out of the package. The `plugins/no-raw-landmarks.grit` rule bans a raw `aside`, `header` or `nav` there, so a prototype takes its nav and rail from `ProposedShell` and its headers from `PaneHeader` or `ExpandedMessage` instead of drawing copies.
