...
SQL-only index objects are real and are precedent. `npm-scripts/pg-search-index.sql` and `npm-scripts/sqlite-search-index.sql` are checked-in, out-of-schema DDL applied idempotently by the migrator, deliberately kept out of the drizzle schema, with paired per-dialect tests. The Postgres file's header is the clearest existing statement of divergent search reach in the repo. D1 does not need this mechanism — `category` is an ordinary column and takes an ordinary schema index — but the precedent for pushing a predicate into the storage engine is established.
