...
**The rule is enforced by the type system, not by remembering it.** The six placement fields — `status`, `syncStatus`, `abandonedMutation`, `mailboxId`, `uid`, `originalMailboxId`, `originalUid` — are absent from `UpdateMessageInput`, so the general-purpose `update()` cannot carry one. Two doors write a placement and there are no others: `transitionPlacement`, which takes a predicate, and the settle in `updateUid`, which takes no input object and writes what the mail server confirmed. A predicate-less placement write is a compile error, and `message-placement-doors.test.ts` fails the build if that stops being true. This is not belt-and-braces: four separate writers on the branch that introduced this rule each reached for `update()` and none of them was caught by review.

**A redelivered worker asks whether ITS mutation is still outstanding, not whether any is.** A move to Trash records `moving`, a permanent delete records `deleting`, a copy records `moving`; a row carrying the other one belongs to a different mutation, and reading it as merely "unsettled" is how a redelivered COPY issues a second copy the server has no way to undo. The shared `isPlacementUnsettled` predicate answers a different question — "may a dependent operation bind this row" — and belongs to callers like `flag-push` that are waiting on somebody else's work.
