...
`NONEXISTENT` on a RENAME has three readings, and only one of them may destroy anything. Either another client deleted the folder; or this rename already landed and only its settle was lost, in which case a redelivery re-issues RENAME from a path that no longer exists; or the server is saying something this code cannot classify. Treating the second or third as the first destroys the folder and its mail while the folder is alive. The server's own listing separates the first two — present target means this rename landed, and the settle runs against it — and everything it cannot answer is a refused rename, because a rename that failed costs a retry while an unclear answer read as a delete costs the user their mail.

Two things make that hold rather than merely intend it. The listing is read through the account's namespace prefix, since the requested string and the path the server keeps are not the same thing (`Projects` against a stored `INBOX.Projects`) and a raw comparison reads a folder that is plainly there as absent. And the decision is made once, where the evidence is: the removal path is reached only by a distinct error raised at the point the listing held neither path, never by re-reading the server's response code at the call site.
...
So the settle does not re-resolve the subtree. It applies to exactly the rows that carry the intent, and those rows identify themselves. Each transitions on its own conditional write — `fullPath = pendingPath`, `pendingPath` cleared, `synced` — and a row that is in the subtree but never carried the intent is not touched. The failure path and the dismissal use the same set, each row to `failed` and to `synced` respectively.

**Row identity is the pair, not the target alone.** A row carries the rename `oldPath → newPath` when it is `pending`, its `fullPath` is `oldPath` or beneath it under the renamed folder's delimiter, **and** its `pendingPath` is exactly what rebasing that `fullPath` from `oldPath` to `newPath` produces. `pendingPath` at or under `newPath` is not sufficient: two folders can hold targets under one branch — `Work → Projects` in flight while `Other → Projects/Old` is recorded — and a settle matching on the target alone stamps `Other` onto a path this rename never asked for, leaves `Other`'s own rename with nothing left to settle, and produces two rows claiming one path for the sweep to reap and re-insert. Nothing but one intent writes the pair, because the intent is recorded over one subtree in one transaction (D6) and the transition is the only writer of either field (D3), so the pair is an identity rather than a heuristic.

The delimiter is the renamed folder's own, read from the row the settle already holds. Deriving one from whatever the pending set happens to contain strands every descendant in the case where the named row has gone.
...
**The named folder settles last.** The redelivery's guard reads that row and nothing else, so it is the witness for the whole settle: while it still carries the intent, a redelivery re-enters and finishes whatever the last attempt left. Settling it first would ack a run that died part-way through its descendants, and those rows would stay `pending` forever — invisible to message sync, and with no route out, because an intent may only be recorded from `synced` or `failed`, so neither a retry nor a dismissal can reach them.

**Nothing after the server RENAME may reach the failure path.** Once the server has executed the command the rename has happened, so a settle that cannot finish — a database failure mid-way, a dependent write that throws — is not a refused rename. Marking the rows `failed` there would offer a retry of work already done and leave `fullPath` naming a path the server no longer holds. It is raised as its own kind, distinguishable from a refusal, and rethrown rather than swallowed: the rows are still `pending` with their targets recorded, so the redelivery's guard passes and the settle runs again. Work that is display-only — the paths recorded beside role appointments — is logged and dropped instead, because failing the job over a label would be worse than the stale label.
