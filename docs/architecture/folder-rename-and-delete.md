...
`NONEXISTENT` on a RENAME has two readings, and they call for opposite actions. Either another client deleted the folder, or this rename already landed and only its settle was lost, in which case a redelivery re-issues RENAME from a path that no longer exists. Treating the second as the first destroys the folder and its mail while the folder is alive at the target. The server separates them: the rename asks whether the target is on it. Present means this rename landed, and the settle runs against it; absent means the folder really is gone.
