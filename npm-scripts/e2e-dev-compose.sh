...
#
# Each Mailpit needs two of them, not one: submission is a separate listener from
# the HTTP API, the app is a host process on this lane so it cannot reach either
# over the compose network, and the suite reads the accepted bytes over the API.
# The lane that refuses is a second Mailpit and so a second pair.
e2e_dev_slot_ports() {
