...
# exists because the suite has moved once already (#445 put it under
# `packages/`) and a half-applied move is otherwise silent.
set -euo pipefail

E2E_SUITE_DIR="packages/e2e"
...
#
# The second is that the suite is itself a workspace of the root project, which
# is what puts it under `npm run typecheck` (#440, #445). npm walks up from the
# working directory, finds the manifest that declares this package a workspace,
# and takes that as the project — so a bare `npm ci` here installs the whole
# monorepo and runs its postinstall. `--prefix` names the project outright and is
# not subject to that walk.
e2e_install() {
	cd "$E2E_DIR"
