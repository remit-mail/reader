 ...
 * every name the harvest guard would keep survives, no row is removed, and a
...
describe("clearing display names that claim another address", () => {
	...
	test("check writes nothing and counts what repair would clear", async () => {
		const report = await sweepDisplayNames(client, "check");

		assert.equal(report.impersonating, 6);
		assert.deepEqual(
	...
	test("repair clears exactly those rows", async () => {
		const report = await sweepDisplayNames(client, "repair");

		assert.deepEqual(
			report.sites.map((site) => [site.table, site.cleared]),
			[
				["address", 3],
				["envelope_address", 1],
				["thread_message", 2],
	...
	test("blanks a name that is, or carries, another address", () => {
		for (const id of ["spoof", "embedded", "tabbed"]) {
			assert.equal(nameOf(id), "", id);
		}
	});
	...
	test("rebuilds the search compound the way the app writes it", () => {
		const row = sqlite
			.prepare(
				"SELECT normalized_compound AS c, normalized_email AS e FROM address WHERE address_id = 'spoof'",
			)
			.get() as { c: string; e: string };
		assert.equal(row.c, row.e);
	});
	...
	test("a second run writes nothing", async () => {
		const report = await sweepDisplayNames(client, "repair");
		assert.equal(report.impersonating, 0);
	});
});

/**
 * The SQL in this repair narrows; it never decides. That only holds if no name
 * the predicate refuses can slip past `LIKE '%_@_%.__%'` — the case where a
 * planted name would survive the sweep unseen.
 */
describe("the SQL narrowing is a superset of the predicate", () => {
	test("selects every name the predicate refuses", () => {
		...
		const selected = new Set(
			(
				sqlite
					.prepare("SELECT name FROM probe WHERE name LIKE '%_@_%.__%'")
		...
		for (const name of names) {
			if (!isImpersonatingDisplayName(name, "nobody@example.org")) continue;
