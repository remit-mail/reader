...
import { createLogger, withTelemetry } from "@remit/remit-logger-lambda";
 ...
 * (#1247). Ticks at the online-tier cadence (rate schedule, wired in
 * infra/stacks/dev/stacks/remit-worker-stack.ts) and delegates the actual
 * decision + enqueue to `runSchedulerTick` — the same function the local
 * dev-stack timer loop calls (see `local-runner.ts`), so production and
 * local dev run one code path.
 *
 ...
 * distinct scheduled firing gets a `time` exactly `onlineIntervalMs` apart
...
export const handler: ScheduledHandler = withTelemetry(async (event) => {
	...
	await runSchedulerTick({
		...
		log,
		onlineIntervalMs: getOnlineIntervalMs(),
