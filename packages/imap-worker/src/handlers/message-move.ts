...
/**
 * SEARCH a mailbox for the copy of a message this operation just put there,
 * identified by its RFC822 Message-ID header. Read-only (EXAMINE, not SELECT) —
 * this is a verification probe, never a write. `null` when nothing matched.
 *
 * Answers the HIGHEST matching uid, and every probe in this package wants that
 * one: uids ascend with arrival inside a UIDVALIDITY, so a copy that has just
 * landed outranks every copy of the same Message-ID the folder already held.
 ...
 */
export const searchMailboxForFreshCopyUid = async (
