...
describe("deriveSenderTrust", () => {
	...
	it("ignores trusted flag (orthogonal axis)", () => {
		const flags: AddressFlags = { trusted: { value: true } };
		assert.equal(deriveSenderTrust(flags), "unknown");
	});
