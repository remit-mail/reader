...
export class BodySyncService {
	...
	private async applyPostStoreSteps(
		...
		// over the just-recomputed one, the same rule `backfillClassification`
		// uses. It likewise protects a `flags.category` override (issue #299):
		// recomputing would let a *later* override silently rewrite a category
		// already decided, which is exactly the churn RFC 030's GSI-safety
		// argument forbids.
		//
		...
		// first store.
		const finalCategory =
			existingMessage.category !== undefined &&
			existingMessage.category !== MessageCategory.uncategorized
	...
	private async backfillClassification(
		...
		// Category is write-once (RFC 034 D3.1): any real category is final;
		// only `uncategorized` (and the absent-column legacy case) need classifying.
		// The hasDecidedCategory/hasDecidedPlacement/hasClassifiedBody helpers were
		// deleted (issue #1011): the re-store path no longer re-enters these
		// derivations at all.
		if (
	 ...
	 * (`applyPostStoreSteps`, `backfillClassification`) already gate on a
	 * decided category (RFC 034 D3.1) before this result reaches a write, so a
	 * message that already carries a real category is never re-touched
	...
	private async applyUnsubscribedAutoRead(
		...
		// Write-once (issue #499): a row that already carries `bodyStorageKey`
		// comes from a completed pass, so its read state was already decided —
		// never re-decide it. (The re-store paths skip this step entirely,
		// issue #1011.)
		if (storedBodyKey) return;
