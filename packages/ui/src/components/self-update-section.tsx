...
export interface SelfUpdateSectionProps {
	...
	onInstall: () => void;
	/** Clears a finished result (success or rollback) from the pane. */
					...
					<div className="space-y-3">
						<div className="flex flex-wrap items-center gap-2">
							...
							<span className="text-2xs text-fg-subtle">
								released {formatReleaseDate(state.release.releasedAt)} · you are
								on {state.version}
							</span>
						</div>
						...
						<p className="text-xs text-fg-subtle">
							...
							minute. If the new version does not come back up, the one you are
							running now is restored on its own.
						</p>
						<div className="flex flex-wrap items-center gap-2">
							<Button
								size="sm"
			...
			{state.status === "rolledBack" && (
				<SectionRow tone="danger">
					<div className="space-y-3">
						<div className="flex items-start gap-2">
							...
							<div className="min-w-0 space-y-1">
								<p className="text-sm font-semibold text-fg">
									Remit {state.attemptedVersion} did not start. Your old version
									was restored.
								</p>
								<p className="text-sm text-fg-muted">
									You are back on {state.version} and everything works. Nothing
									was lost.
								</p>
							</div>
						</div>
						<div className="space-y-1">
							<p className="text-xs text-fg-subtle">Why it failed</p>
						...
						<div className="space-y-1">
							<p className="text-xs text-fg-subtle">
								The full log from the attempt is on your server:
							</p>
